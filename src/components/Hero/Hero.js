...
const Hero = () => {
  ...
  //TODO: делай деструктуризацию
  const movie = useSelector(({selectedMovie}) => selectedMovie.selectedMovie)
  // TODO: почитать про логические операторы
  return (
    movie ?
      <div className={s.hero}
           style={{backgroundImage: movie?.backdrop_path ? `url('${IMAGE_PATH}${movie?.backdrop_path}')` : `url('${default_hero}')`}}>

        <div className={s.item} key={movie?.id}>

          <h1 className={s.title}>{movie?.title}</h1>

          {/*TODO: выносить ивенты в функции*/}
          {!movie?.videos?.results.length ? null :
            <button className={s.playBtn} onClick={() => setPlayTrailer(true)}>
            Play trailer <BsFillPlayBtnFill className={s.icon}/>
          </button>}
          ...
          <div className={s.bottom_content}>
            ...
            <MovieGenres genresId={movie?.genres?.map(genre => genre.id)} className={s.genreItem}/>

            {movie?.runtime ? <div className={s.runtime}>{movie?.runtime} min</div> : null}
      ...
      </div> : <img className={s.default_hero} src={default_hero} alt={'default_hero'}/>)
}
